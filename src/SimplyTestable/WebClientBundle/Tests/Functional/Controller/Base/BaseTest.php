...
abstract class BaseTest extends BaseSimplyTestableTestCase
{
    ...
    protected function getCurrentController($postData = null, $queryData = null)
    {
        ...
        return $this->getRoutedController(
    ...
    protected function getControllerNameFromTestNamespace()
    {
        ...
        $nonActionNamespaceParts = array_slice(
            $testNamespaceParts,
            0,
            array_search(ucfirst($this->getActionName()), $testNamespaceParts)
        );

        return str_replace('\Tests\\Functional\\', '\\', implode('\\', $nonActionNamespaceParts) . 'Controller');
    }
    ...
    {
        $routeActionName = str_replace('Action', '', $this->getActionNameFromTestNamespace());

        return strtolower(implode('_', $this->getControllerRelatedNamespaceParts()) . '_' . $routeActionName);
    }
